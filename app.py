...
if uploaded_file is not None:
    ...
    with main_tab2:
        ...
        st.dataframe(styled_t2, use_container_width=True)import streamlit as st
import pandas as pd
import numpy as np
import plotly.graph_objects as go
from plotly.subplots import make_subplots
...
def evaluate_conditions(df, time_col, start_dt, end_dt, conditions):
    ...
    for idx, cond in enumerate(conditions):
        ...
        if op == ">": c_mask = (df[col] > val)
        elif op == "<": c_mask = (df[col] < val)
        elif op == "=": c_mask = (df[col] == val)
        elif op == ">=": c_mask = (df[col] >= val)
        elif op == "<=": c_mask = (df[col] <= val)
        elif op == "!=": c_mask = (df[col] != val)
        else: c_mask = pd.Series(True, index=df.index)
...
if uploaded_file is not None:
    ...
    with main_tab1:
        ...
        if 'filter_conditions_t1' not in st.session_state:
            st.session_state.filter_conditions_t1 = [
                {'id': str(uuid.uuid4()), 'logic_op': 'AND', 'column': available_columns[0], 'operator': '>', 'value': 0.0}
            ]
        ...
        for idx, cond in enumerate(st.session_state.filter_conditions_t1):
            ...
            scol = c_col.selectbox(f"Parametr #{idx+1}", available_columns,
                                   index=available_columns.index(cond['column']) if cond['column'] in available_columns else 0,
                                   key=f"t1_col_{cid}")
            sop = c_op.selectbox(f"Warunek #{idx+1}", [">", "<", "=", ">=", "<=", "!="],
                                 index=[">", "<", "=", ">=", "<=", "!="].index(cond['operator']),
                                 key=f"t1_op_{cid}")
        ...
        if to_remove_t1:
            st.session_state.filter_conditions_t1 = [c for c in st.session_state.filter_conditions_t1 if c['id'] != to_remove_t1]
            st.rerun()

        exec_t1 = st.button("🚀 Wykonaj zapytanie i przefiltruj", type="primary", key="exec_t1", use_container_width=True)

        if 'filtered_df_t1' not in st.session_state:
            st.session_state.filtered_df_t1 = data.copy()

        if exec_t1:
            mask_t1 = evaluate_conditions(data, time_column, start_dt_t1, end_dt_t1, st.session_state.filter_conditions_t1)
            st.session_state.filtered_df_t1 = data[mask_t1].copy().reset_index(drop=True)
        ...
        if selected_params_t1 and not df_t1.empty:
            ...
            if chart_mode_t1 == "Subplots (Osobne)":
                ...
                for param in selected_params_t1:
                    ...
                    fig_t1.add_trace(go.Scatter(x=df_t1[time_column], y=y_plot, mode='lines', name=param, customdata=y_data, hovertemplate=ht))
            ...
            # Odczyt parametrów z punktu wskazówki
            st.markdown(f"##### 📊 Odczyt parametrów z punktu czasowego `{pd.to_datetime(actual_dt_t1).strftime('%Y-%m-%d %H:%M:%S')}`:")
            readout_cols = st.columns(min(len(selected_params_t1), 4))
            for i, param in enumerate(selected_params_t1):
                val = found_row_t1[param]
                readout_cols[i % 4].metric(label=param, value=f"{val:.2f}" if isinstance(val, (int, float)) else str(val))
        ...
        if chart_sel_mask_t1.sum() > 0:
            st.success(f"🎯 **Zaznaczono na wykresie {chart_sel_mask_t1.sum()} rekordów.** Odpowiednie komórki/wiersze zostały wyróżnione na złoto poniżej.")
        else:
            st.caption("💡 *Wskazówka: Zaznacz obszar na wykresie powyżej za pomocą narzędzia Box Select lub Lasso Select, aby podświetlić te dane w tabeli.*")
        ...
        def style_t1_table(df, sel_mask):
            ...
            for idx in df.index:
                if sel_mask.loc[idx]:
                    # Zaznaczony obszar z wykresu: Złote tło z kontrastową ramką
                    styles.loc[idx] = 'background-color: #FFF59D; color: #B71C1C; font-weight: bold; border: 2px solid #F57F17;'
                else:
                    # Estetyczny, naprzemienny niebieski styl dla Trybu 1
                    if idx % 2 == 0:
                        styles.loc[idx] = 'background-color: #F0F7FF; color: #0D47A1; border: 1px solid #D0E1F9;'
    ...
    with main_tab2:
        st.header("Wyróżnianie wartości na tle wszystkich danych")
        st.info("💡 Wszystkie dane są widoczne w jednym miejscu. Rekordy spełniające filtry są oznaczone na zielono, a obszar zaznaczony na wykresie – na złoto.")
        ...
        if to_remove_t2:
            st.session_state.filter_conditions_t2 = [c for c in st.session_state.filter_conditions_t2 if c['id'] != to_remove_t2]
            st.rerun()

        exec_t2 = st.button("🚀 Wykonaj zapytanie i oznacz dane", type="primary", key="exec_t2", use_container_width=True)

        if 'applied_mask_t2' not in st.session_state or len(st.session_state.applied_mask_t2) != len(data):
            st.session_state.applied_mask_t2 = evaluate_conditions(data, time_column, start_dt_t2, end_dt_t2, st.session_state.filter_conditions_t2)
        ...
        st.markdown(f"**Wyróżniono {matched_count} z {len(data)} rekordów** (Dopasowanie: {matched_count/len(data)*100:.1f}%)")
        ...
        if selected_params_t2:
            ...
            with c_info2:
                status_text = "🎯 SPEŁNIA WARUNKI" if is_matched_dt else "⚪ STAN STANDARDOWY"
                st.info(f"📍 **Wskazówka na:** `{pd.to_datetime(actual_dt_t2).strftime('%Y-%m-%d %H:%M:%S')}` | **Status:** `{status_text}`")
            ...
            st.markdown(f"##### 📊 Odczyt parametrów z punktu czasowego `{pd.to_datetime(actual_dt_t2).strftime('%Y-%m-%d %H:%M:%S')}`:")
            readout_cols2 = st.columns(min(len(selected_params_t2), 4))
            for i, param in enumerate(selected_params_t2):
                val = found_row_t2[param]
                readout_cols2[i % 4].metric(label=param, value=f"{val:.2f}" if isinstance(val, (int, float)) else str(val))
        ...
        if chart_sel_mask_t2.sum() > 0:
            st.success(f"🎯 **Zaznaczono na wykresie {chart_sel_mask_t2.sum()} rekordów.** Odpowiednie wiersze zostały wyróżnione poniżej.")
        else:
            st.caption("💡 *Użyj narzędzia zaznaczania (Box Select / Lasso Select) na wykresie powyżej, aby dynamicznie podświetlić konkretne wiersze w tabeli.*")
        ...
        def style_t2_table_combined(df, match_mask, sel_mask):
            ...
            for idx in df.index:
                ...
                if is_m and is_s:
                    # Spełnia warunek ORAZ zaznaczony na wykresie -> Złoto-czerwony z grubą obramówką
                    styles.loc[idx] = 'background-color: #FFF176; color: #880E4F; font-weight: bold; border: 3px solid #D50000;'
                elif is_s:
                    # Zaznaczony tylko na wykresie -> Złoty z pomarańczową obramówką
                    styles.loc[idx] = 'background-color: #FFF59D; color: #E65100; font-weight: bold; border: 2px solid #F57C00;'
                elif is_m:
                    # Spełnia warunek filtru -> Wyrazista zielona ramka i jasnozielone tło
                    styles.loc[idx] = 'background-color: #E8F5E9; color: #1B5E20; font-weight: bold; border: 2px solid #2E7D32;'
                else:
                    # Standardowy wiersz
                    styles.loc[idx] = 'background-color: #FAFAFA; color: #757575; border: 1px solid #E0E0E0;'
            return styles

        styled_t2 = df_display_t2.style.apply(lambda r: style_t2_table_combined(df_display_t2, mask_t2, chart_sel_mask_t2).loc[r.name], axis=1)
